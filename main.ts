...
import spellCheckPlugin from "./src/plugins/spellCheck/index";
...
app.whenReady().then(() => {
  ...
  ipcMain.handle("export-pdf", async (_event, content: string) => {
    ...
    try {
      const pdfPath = await kernel.executePlugin(
        "pdf-export",
        content,
        result.filePath
      );
  ...
  // Thêm vào phần đăng ký plugin
  kernel.registerPlugin(spellCheckPlugin.name, spellCheckPlugin);

  // Thêm IPC handler cho spell check
  ipcMain.handle("spell-check", async (_event, content: string) => {
    try {
      return await kernel.executePlugin("spell-check", content);
    } catch (error) {
      console.error("Error executing spell-check plugin:", error);
      return null;
    }
  });
});
